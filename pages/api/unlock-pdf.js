  ...
  if (contentType && contentType.indexOf("multipart/form-data") !== -1) {
    ...
    if (!result.files || !result.fields) {
      ...
      if (result.files.file && result.files.file.newFilename) {
        ...
        //Get password
        const passwordToUnlock = result.fields.password;
        ...
        if (file != null) {
          ...
          if (exist) {
            const command = `qpdf --password="${passwordToUnlock}"  --decrypt ${file.inputFilePath} ${file.outputFilePath}`;

            //Run command to unlock the pdf file
