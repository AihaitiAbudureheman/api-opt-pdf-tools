  ...
  if (contentType && contentType.indexOf("multipart/form-data") !== -1) {
    ...
    if (!result.files || !result.fields) {
      ...
      if (result.files.file && result.files.file.newFilename) {
        ...
        const outputFileExtension = ".pptx";
        ...
        if (file != null) {
          ...
          if (originalFileData) {
            //conversion commad
            const command = `soffice --infilter="impress_pdf_import" --convert-to pptx:"Impress MS PowerPoint 2007 XML" --outdir ${file.pdfToolsFilesPath} ${file.inputFilePath}`;
